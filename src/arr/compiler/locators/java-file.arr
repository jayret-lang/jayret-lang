    ...
    var ast = nothing
    {
      ...
      method get-module(self) block:
        when ast == nothing block:
          ...
          str = f.read-file()
          f.close-file()
          ast := CL.pyret-ast(PJ.surface-parse(str, self.uri()))
        end
        ast
      end,
      ...
      method uri(self): "java-file://" + string-replace(file-ops.real-path(self.path), P.path-sep, "/") end,
...
java-file-locator = mockable-java-file-locator({
